...
$query = $dbh->prepare("SELECT * FROM products LEFt JOIN suppliers on products.idf = suppliers.idf");
$query->execute();
                                    ...
                                    <table id="clicktoedit" class="table table-bordered table-striped">
										<thead class="bg-inverse">
											<tr class="text-center">
												...
												<th class="text-center">Full/Empty</th>
...
foreach($result as $row){
    ...
    if (($row['quantity'])<($row['minp'])) {
        $colo = 'grey';
    }
    elseif ($row['quantity']>($row['maxp'])){
        $colo ='Crimson ';
    }
    else {
        $colo ='DarkCyan';
    ...
    $id=$row['idpdct'];
									...
									echo	"<tbody>";
									echo	"<tr>";
									echo	"<td tabindex='1' class='text-center'>".$row['idpdct']."</td>";
                                    echo    "<td tabindex='1' class='text-center'>".$row['namep']."</td>";
                                    echo    "<td tabindex='1' class='text-center'>".($row['namesup'] != "" ? $row['namesup'] : "None" )."</td>";
									echo	"<td tabindex='1' class='text-center'>".$row['descri']."</td>";
									echo	"<td tabindex='1' class='text-center'>".$row['pic1']."</td>";
                                    echo    "<td tabindex='1' class='text-center'>".$row['category']."</td>";
									echo	"<td class='text-center'>".$row['price']."</td>";
									echo	"<td class='text-center'>".$row['quantity']."</td>";
									echo	"<td class='text-center' style='background-color:$colo'></td>";
									echo	"<td class='text-center'>
													<div class='dropdown'>
														...
														<ul class='dropdown-menu panel-settings'>
															<li>
																<a  href='./updatepdt.php?id=".$id."' class='reloadBar'>
																	Update
																</a>
															</li>
															<li role='separator' class='divider'></li>
															<li>
																<a  href='./deletepdt.php?id=".$id."' class='reloadBar'>
																	Delete
																</a>
															</li>
															<li role='separator' class='divider'></li>
															<li>
																<a  href='./neworder.php?id=".$id."' class='reloadBar'>
																	Order
																</a>
															</li>
														</ul>
    ...
    ?>
                                ...
                                <div class="panel-footer">
                                    <a class="btn btn-info add-row" href="newpdt.php">
                                        <button  type="button" class="btn btn-info add-row" data-sort="true"> Add Product  </button>
                                    </a>
                                </div>
                            </div>
                        </div>
